 ...
 var day = moment.tz("Asia/Ho_Chi_Minh").day();
  ...
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      console.log('❀ 𝐂𝐇𝐄𝐂𝐊 𝐓𝐔𝐎𝐍𝐆 𝐓𝐀𝐂 𝐃𝐀𝐘 𝐍𝐄𝐖 ❀');
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '❀ 𝐓𝐎𝐏 𝟏𝟎 𝐓𝐮𝐨𝐧𝐠 𝐓𝐚𝐜 𝐍𝐠𝐚𝐲 ❀\n';
        checkttBody += storage.slice(0, 10).map(item => {
          return `『𝐍𝐚𝐦𝐞』:${count++}. ${item.name} \n•『 𝐒𝐨̂́ 𝐓𝐢𝐧 𝐍𝐡𝐚̆́𝐧』 : ${item.count} •`;
      }).join('\n');
      ...
      if (day_now == 1) {
        console.log('❀ 𝐂𝐇𝐄𝐂𝐊 𝐓𝐔𝐎𝐍𝐆 𝐓𝐀𝐂 𝐖𝐄𝐄𝐊 𝐍𝐄𝐖 ❀');
        checkttData.forEach(async(checkttFile) => {
          const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
          let storage = [], count = 1;
          ...
          let checkttBody = '❀ 𝐓𝐎𝐏 𝟏𝟎 𝐓𝐮𝐨𝐧𝐠 𝐓𝐚𝐜 𝐓𝐮𝐚𝐧 ❀\n';
          checkttBody += storage.slice(0, 10).map(item => {
            return `『𝐍𝐚𝐦𝐞』 :${count++}. ${item.name} \n•『 𝐒𝐨̂́ 𝐓𝐢𝐧 𝐍𝐡𝐚̆́𝐧』: ${item.count} •`;
        }).join('\n');
          api.sendMessage(checkttBody, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
(async function () {
 ...
 //////////////////////////////////////////////
    ...
    const handleUnsend = require("./handle/handleUnsend")({ api, Users });
    const handleCreateDatabase = require("./handle/handleCreateDatabase")({ api, Threads, Users, Currencies, models });
    const a = [], txt = ["png", "jpg", "mp4", "jpeg", "gif", "m4a", "mp3"];
  for (type of txt) {
    a.push(type);
    const fileb = fs.readdirSync(`./modules/commands/cache`).filter(file => file.endsWith(`.` + type));
    for (filec of fileb) {
      try {
        fs.unlinkSync(`./modules/commands/cache/` + filec)
      }
      catch {
        logger("Lỗi khi xóa tập tin: " + filec, "[ ❗ ]")
      }
    }
  };
  logger(`Đã xóa các tập tin có đuôi: ${a.join(", ")}`, "[ CLEAR ]");
...
//    if (event.type == "change_thread_image") api.sendMessage(`» [ 𝗖𝗔̣̂𝗣 𝗡𝗛𝗔̣̂𝗧 𝗡𝗛𝗢́𝗠 ] ${event.snippet}`, event.threadID);
    ...
    if (!find_thuebot)return api.shareContact('⛔ Nhóm của bạn chưa thuê bot, Vui lòng thuê bot để tiếp tục sử dụng',global.config.ADMINBOT[0],event.threadID);
    if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()+25200000)return api.shareContact(`⚠️ Nhóm của bạn đã hết hạn thuê bot vui lòng thanh toán để tiếp tục gia hạn`,global.config.ADMINBOT[0],event.threadID);
    };
    }
      let unsend = __dirname + "/../modules/commands/cache/unsendReaction.json";
