...
//// admin -pa /////
    ...
    if (userBanned.has(senderID) || threadBanned.has(threadID) || allowInbox == ![] && senderID == threadID) {
      if (!ADMINBOT.includes(senderID.toString()) && !NDH.includes(senderID)) {
 ...
 return api.unsendMessage(info.messageID);
    ...
    if (!command) {
      ...
      else return api.shareContact(global.getText("handleCommand", "commandNotExist", checker.bestMatch.target),event.senderID, event.threadID);
    }
    ...
    if (command.config.hasPermssion > permssion) return api.sendMessage(global.getText("handleCommand", "permssionNotEnough", command.config.name), event.threadID, event.messageID);
      ...
      return api.shareContact(`⏱ Bạn đang trong thời gian chờ!\n Vui lòng thử lại sau ${((timestamps.get(senderID) + expirationTime - dateNow)/1000).toString().slice(0, 5)}s nữa nhé`, event.senderID, event.threadID);
